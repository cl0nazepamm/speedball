...
function asMaterialNormalNode( node ) {

	if ( node && node.isMaterialXNormalMapNode ) return node;
	return transformNormalToView( node );
...
function getInputDefinition( inputs, name ) {

	return inputs.__materialXInputs ? inputs.__materialXInputs[ name ] : null;

}

function getAuthoredNumber( inputs, name ) {

	const input = getInputDefinition( inputs, name );
	if ( ! input || input.value === null ) return null;
	const value = Number( input.value );
	return Number.isFinite( value ) ? value : null;

}

function getWeightedScalarInput( inputs, name, fallback ) {

	const node = inputs[ name ];
	const authoredValue = getAuthoredNumber( inputs, name );
	const input = getInputDefinition( inputs, name );

	if ( hasNodeValue( node ) === false ) return fallback;
	if ( input && input.hasReference && authoredValue !== null ) {

		if ( Math.abs( authoredValue ) <= 1e-6 ) return float( 0 );
		if ( Math.abs( authoredValue - 1 ) > 1e-6 ) return mul( float( authoredValue ), node );
...
function applyStandardSurface( material, inputs, issueCollector, nodeName ) {
	...
	if ( clearcoatEnabled && hasNodeValue( inputs.coat_normal ) ) {
		...
		material.clearcoatNormalNode = asMaterialNormalNode( inputs.coat_normal );

	}

	if ( hasNodeValue( inputs.normal ) ) material.normalNode = asMaterialNormalNode( inputs.normal );
...
function applyGltfPbrSurface( material, inputs, issueCollector, nodeName ) {
	...
	if ( hasNodeValue( inputs.clearcoat_normal ) ) {
		...
		material.clearcoatNormalNode = asMaterialNormalNode( inputs.clearcoat_normal );
...
function applyOpenPbrSurface( material, inputs, issueCollector, nodeName ) {
	...
	const baseWeight = getWeightedScalarInput( inputs, 'base_weight', float( 1 ) );
	const baseColor = inputs.base_color || color( 0.8, 0.8, 0.8 );
	const baseMetalness = getWeightedScalarInput( inputs, 'base_metalness', undefined );
	const specularWeight = getWeightedScalarInput( inputs, 'specular_weight', undefined );
	const specularAnisotropy = getWeightedScalarInput( inputs, 'specular_roughness_anisotropy', undefined );
	const coatWeight = getWeightedScalarInput( inputs, 'coat_weight', undefined );
	const fuzzWeight = getWeightedScalarInput( inputs, 'fuzz_weight', undefined );
	const transmissionWeight = getWeightedScalarInput( inputs, 'transmission_weight', undefined );
	const thinFilmWeight = getWeightedScalarInput( inputs, 'thin_film_weight', undefined );
	const emissionLuminance = getWeightedScalarInput( inputs, 'emission_luminance', float( 0 ) );
	const coatEnabled = isEnabledWeightNode( coatWeight );
	const fuzzEnabled = isEnabledWeightNode( fuzzWeight );
	const transmissionEnabled = isEnabledWeightNode( transmissionWeight );
	const thinFilmEnabled = isEnabledWeightNode( thinFilmWeight );
	material.colorNode = mul( baseWeight, baseColor );

	if ( hasNodeValue( baseMetalness ) && isEffectivelyZero( baseMetalness ) === false ) {
	...
	if ( hasNodeValue( specularWeight ) && isEffectivelyOne( specularWeight ) === false ) {

		material.specularIntensityNode = specularWeight;
	...
	setAnisotropy( material, specularAnisotropy, float( 0 ) );

	if ( coatEnabled ) {

		const coatWeightNode = coatWeight || float( 0 );
		...
		if ( hasNodeValue( inputs.geometry_coat_normal ) ) {
			...
			material.clearcoatNormalNode = asMaterialNormalNode( inputs.geometry_coat_normal );
	...
	if ( transmissionEnabled ) {
		...
		material.transmissionNode = transmissionWeight;
	...
	if ( hasNodeValue( inputs.geometry_normal ) ) material.normalNode = asMaterialNormalNode( inputs.geometry_normal );

	if ( thinFilmEnabled ) {

		material.iridescenceNode = thinFilmWeight;
	...
	setTransmissionFlags( material, transmissionWeight, inputs.geometry_opacity );
