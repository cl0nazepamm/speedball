...
const toVec3 = ( node ) => vec3( element( node, 0 ), element( node, 1 ), element( node, 2 ) );

const getTextureDefaultNode = ( nodeX ) => {

	const defaultNode = nodeX.getNodeByName( 'default' );
	if ( defaultNode !== undefined && defaultNode !== null ) return defaultNode;

	if ( nodeX.type === 'float' ) return float( 0 );
	if ( nodeX.type === 'color3' || nodeX.type === 'vector3' ) return vec3( 0, 0, 0 );
	if ( nodeX.type === 'color4' || nodeX.type === 'vector4' ) return vec4( 0, 0, 0, 1 );
	return vec4( 0, 0, 0, 1 );

};

const getTextureSampleNode = ( sampled, nodeX, file ) => {

	if ( nodeX.type === 'float' ) return element( sampled, 0 );
	if ( nodeX.type === 'vector3' ) return toVec3( sampled );
	if ( nodeX.type === 'vector4' ) return sampled;

	const colorSample = applyTextureColorSpace( sampled, file );
	if ( nodeX.type === 'color3' ) return toVec3( colorSample );
	if ( nodeX.type === 'color4' ) return colorSample;
...
const compileImageLikeNode = ( nodeX, compileContext ) => {
	...
	const fallback = getTextureDefaultNode( nodeX );
	const node = sampleTexture( textureFile, uvNode, compileContext, fallback );
	return textureFile ? getTextureSampleNode( node, nodeX, file ) : fallback;
...
const compileTiledImageNode = ( nodeX, compileContext ) => {
	...
	if ( ! textureFile ) {
		...
		return getTextureDefaultNode( nodeX );
	...
	const fallback = getTextureDefaultNode( nodeX );
	const node = sampleTexture( textureFile, transformedUv, compileContext, fallback );
	return getTextureSampleNode( node, nodeX, file );
...
function compileNodeFromRegistry( nodeX, out, compileContext ) {
	...
	const nodeElement = compileContext.nodeLibrary[ nodeX.element ] || compileContext.nodeLibrary[ nodeX.element.split( '_' )[ 0 ] ];
	if ( ! nodeElement ) {
