...
// three.js r185 dropped the `mx_smoothstep` TSL export that existed on the 185dev
// branch this loader was authored against. Inline the original 185dev implementation
// so MaterialX `smoothstep` keeps its (in, low, high) arg order and degenerate-range
// fallback (stock r185's loader downgrades this node to plain `smoothstep`).
...
const tagMaterialXNormalMap = ( node ) => {

	if ( node && typeof node === 'object' ) {

		try {

			Object.defineProperty( node, 'isMaterialXNormalMapNode', {
				value: true,
				configurable: true,
			} );
			...
			node.isMaterialXNormalMapNode = true;
...
const mx_normalmap = ( inNode = vec3( 0.5, 0.5, 1.0 ), scale = 1 ) => {

	if ( inNode && inNode.isMaterialXNormalMapNode ) return inNode;
	return tagMaterialXNormalMap( normalMap( inNode, scale ) );
...
const MXElements = [
	...
	createMXElement( 'normalmap', mx_normalmap, [ 'in', 'scale' ], { in: defaultVec3( 0.5, 0.5, 1.0 ), scale: defaultFloat( 1 ) } ),
	...
	createMXElement( 'worleynoise2d', mx_worley_noise_float, [ 'texcoord', 'jitter', 'style' ], {
		texcoord: defaultVec2( 0, 0 ),
		jitter: defaultFloat( 1 ),
		style: defaultInt( 0 ),
	} ),
	createMXElement( 'worleynoise3d', mx_worley_noise_float, [ 'position', 'jitter', 'style' ], {
		position: () => positionLocal,
		jitter: defaultFloat( 1 ),
		style: defaultInt( 0 ),
	} ),
