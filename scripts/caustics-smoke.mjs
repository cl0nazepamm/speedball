...
const demo = await readFile(new URL('../dispersion.html', import.meta.url), 'utf8');
...
assert.match(demo, /const CAUSTIC_GRID = 768/);
assert.match(demo, /const CAUSTIC_TARGET_PHOTONS = 3_000_000/);
assert.match(demo, /const CAUSTIC_PHOTONS_PER_FRAME = 90_000/);
assert.match(demo, /const CAUSTIC_RESOLVE_INTERVAL = 2/);
assert.match(demo, /grid: CAUSTIC_GRID/);
assert.match(demo, /targetPhotons: CAUSTIC_TARGET_PHOTONS/);
assert.match(demo, /photonBudget: params\.causticPhotons/);
assert.match(demo, /resolveInterval: CAUSTIC_RESOLVE_INTERVAL/);
assert.match(demo, /fCau\.add\(params, 'causticPhotons', 30000, 180000, 5000\)/);
...
assert.match(demo, /caustics\.setLightIntensity\(params\.spotIntensity\)/);
assert.match(demo, /caustics\.setLightColor\(spot\.color\.r, spot\.color\.g, spot\.color\.b\)/);
assert.match(demo, /caustics\.setReceiverAlbedo\(floorMat\.color\.r, floorMat\.color\.g, floorMat\.color\.b\)/);
assert.match(demo, /depthWrite: false/);

// Mesh exit rays use a threaded BVH rather than an O(triangle-count) scan.
assert.match(engine, /buildCausticBvh\(wpos, idxArr\)/);
...
assert.match(demo, /caustics\.setLightCone\(_spotDir\.subVectors\(spot\.target\.position, spot\.position\), spot\.angle, spot\.penumbra\)/);
...
assert.doesNotMatch(demo, /m\.thickness = params\./);
assert.match(demo, /causticAbsorption: 0\.35/);
assert.match(demo, /name\('absorption'\)\.onChange\(\(v\) => caustics\.setThickness\(v\)\)/);
...
assert.match(readme, /BVH-accelerated through-mesh Snell/);
