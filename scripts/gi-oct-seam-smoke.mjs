...
// CPU analogue of one 6x6 oct tile plus its canonical 1px mirrored gutter.
// A smooth asymmetric field makes either fold-axis discontinuity measurable.
const OCT = 6;
const BORDER = 1;
...
assert.ok(xFoldJump < 1e-5, `x fold must be continuous, got ${xFoldJump}`);
assert.ok(yFoldJump < 1e-5, `y fold must be continuous, got ${yFoldJump}`);
